...
def setup_module():
    with io.open('tests/register_count.bin', 'wb') as f:
        data = struct.pack('d', 11.57)
        f.write(data)
    with io.open('tests/menu.txt', 'w') as f:
        f.write(u"#Candy|1.00\n001|Chocolate bar\n002|Gum|0.75\n" +
                u"#Beverage|0.50\n003|Hot chocolate|0.50|hc\n")
    with io.open('tests/employees.txt', 'w') as f:
        f.write(u"Admin|2222|admin|2\nEmployee|1111|employee|1\n" +
                u"Guest|0000|guest|0\n")


def teardown_module():
    os.remove('tests/menu.txt')
    os.remove('tests/employees.txt')
    os.remove('tests/register_count.bin')


def test_register_reads_menu():
    register = Register('tests/menu.txt', 'tests/employees.txt',
                        'tests/register_count.bin')
    menu = register.menu
    correct_menu = [
...
def test_register_checkout_order():
    ...
    assert register.order == {}
    with io.open('tests/register_count.bin', 'rb') as f:
